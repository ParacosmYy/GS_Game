...
本文定义一个角色何时算“完整”。当前以 Kyo 作为第一样板定义，Ryo 继续作为 baseline 与合同参照，Phase 2 的其他角色复用同一套标准。
...
Phase 1 基线达标后，Ryo / Iori / 其他角色使用同一份完整性标准；Phase 2 的角色完整度报告应该同时显示多角色差距，而不是只显示单个角色。
