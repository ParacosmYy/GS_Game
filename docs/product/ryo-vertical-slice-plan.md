...
本文是 Phase 1 的产品基线，也是 Phase 2 的验收参照。Phase 2 的迭代仍然要服从这条线建立起来的内容、动作和反馈合同，但不再只围着 Ryo 单线推进。

当前的执行口径更进一步：

- 先把一个人物做精，再做其他人物。
- 先把 Kyo 做成第一套可复制样板，再让 Ryo 继续作为 baseline 与合同参照。
- 任何会让内容再次横向散落的改动，都要先证明不会破坏这条主线或 Phase 2 收口。
...
因此当前阶段先收敛到 Kyo 第一样板，再把 Ryo / Iori 和流程仪式感推进到可复制模板。

当前最重要的推进顺序见 [KOF 差距矩阵](kof-gap-matrix.md)。本计划负责告诉我们“Kyo 第一样板是什么”，差距矩阵负责告诉我们“Phase 2 先补哪一块最值钱”。
...
- Kyo 的站姿、移动、攻击、受击、倒地有一致气质。
- Kyo 的肖像不再是色块或临时图形。
- Kyo 的攻击视觉帧、判定帧、命中反馈能通过同一份 frame contract 对齐。
- Kyo 的资产格式可以复制给 Ryo/Iori/其他角色。
...
当前不是继续扩角色，而是把 Phase 2 的主线从“有入口”推进到“真实内容包 + 真正的流程仪式感”：
...
这一步的目标不是“更多文件”，而是让每一类内容都有固定归属，方便下一轮继续迁移而不再散落。
...
- Ryo / Kyo / Iori 内容包必须持续向真实数据迁移，避免子目录只剩说明文本。

## 5. 每轮推进顺序

1. 定义或修正数据格式。
2. 写校验测试或校验工具。
3. 接入某个角色内容包的一个明确子域（commands/moves/attacks/animations/hitboxes/feedback/portraits/reports 之一），并优先让该子域不再只剩 README。
4. 保留 fallback。
5. 验证构建。
6. 复盘是否更像 KOF。
...
- 如果本轮不是在推进 Phase 2 的明确目标，就不能把它包装成“主线优化”。
- 如果本轮会把内容分散到更多位置，必须先收口再实施。
- 如果本轮只能让文件表面更整齐，但不会让当前阶段更完整，就不算样板推进。

本阶段还必须同步满足：

- 战斗界面显示当前选中角色 moveList。
- 标准爆气说明在 UI 中同时展示 `K+U` 和 `O`。
- 招式提示与角色定义数据一致，不允许 HUD 文案单独漂移。

## 6. 退出条件

Phase 1 基线达到以下条件后，Phase 2 可继续扩展和优化其他角色：
...
1. Kyo：验证火焰特效、快速连段、主角气质，并作为第一复制样板。
