...
本轮用于提交的有效改动必须达到至少 `200` 行变更，以 `git diff --shortstat` 的新增+删除总和为准；如果只做了验证或改动不足该门槛，不得为了满足流程而提交。
...
- 如果本轮涉及文件池或状态机拆分，还必须说明拆分后的职责边界和统一入口。
...
- 只有累计到 5 个“有效 commit”后，才允许打一个 tag；未达到最低改动门槛的提交不计入这 5 次。
