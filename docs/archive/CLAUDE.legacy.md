...
- **每轮迭代结束后必须 commit** — 不允许把多轮迭代合并成一个提交，也不允许把一轮迭代拆成多个提交
- 一次 commit 必须是以下之一：
  - `feat(scope):` 新功能（如新增角色、新系统）
  - `fix(scope):` Bug 修复（含根因说明）
  - `refactor(scope):` 重构（不改行为，只改结构）
  - `chore:` 构建/工具变更
- commit message 必须中文，建议格式为 `type(scope): 中文标题`，正文用分点说明 `原因`、`差异`、`风险`
...
2. **每次迭代 +5 分** — 不多不少。哪怕改了很多也只加5分。从当前分数每次加5分递增。**禁止按多维度累加得分** — 即使一次改了6个维度，总分也只+5。所有维度得分的总和必须等于"上一轮总分+5"
...
10. **每轮迭代前先搜索公开资源** — 先查公开 MUGEN、Ikemen GO、开源格斗项目或其他开放资料，记录可借鉴点后再开始实现
11. **执行顺序固定** — 先走 PM / 用户需求，再走研发实现、测试文档补齐、最后做架构审查，不允许跳步
12. **每轮只加 5 分** — 不按功能数、文件数、修复点数叠加，单轮结算永远只增加 5 分
13. **每轮必须留痕** — 结束前必须 commit，提交信息必须中文，正文分点写清 `原因` 和 `差异`，让后续能快速追踪这轮改动

---

## 十三、正版靠拢实施原则

> 目标不是“像个能跑的游戏”，而是“像一个有街机气质的正版格斗游戏样机”。

### 13.1 先样板，后复制

- 每次只推进一条样板线，默认顺序：Ryo → Kyo → Iori
- 样板线必须同时覆盖：打击感、动作气质、头像气质、胜利表现
- 样板未跑通前，不扩新角色、不扩新模式、不扩新招式

### 13.2 先手感，后数量

- 新增内容必须先满足“命中像重拳、受击像受击、停顿像卡肉”
- 角色数量不作为迭代目标，完成度才是目标
- 如果同一轮只能改一项，优先改打击反馈而不是外观细节

### 13.3 头像与战斗表现统一

- 肖像不是独立装饰，必须和角色站姿、气质、胜利姿势一致
- 每个角色的头像、对战动作、胜利台词要形成同一性格表达
- 头像风格先统一框架，再单角色微调

### 13.4 栈选择原则

- 默认不更换技术栈
- 只有在“当前栈无法稳定表达目标效果”且“有明确收益”时，才考虑升级渲染或工具链
- 任何栈变化都必须先给出对比方案和迁移成本，不能凭感觉重构

### 13.5 每轮必须可验证

- 每次迭代结束必须通过 `npx tsc --noEmit` 和 `npx vite build`
- 每次视觉迭代都要保留 fallback，不能一次性拆掉旧渲染路径
- 每次改动都要能说明：这轮更像正版了哪里，没变好的地方是什么
