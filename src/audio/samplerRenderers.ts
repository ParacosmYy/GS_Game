/**
 * Sampler Render Functions — Pure Audio DSP Renderers
 *
 * All render functions for the sampler system. These are pure functions
 * that take a sample rate (sr) and return a Float32Array of audio samples.
 * They import DSP primitives from samplerRenderUtils.ts.
 *
 * These were extracted from sampler.ts to keep the main sampler module
 * focused on initialization and playback, while render logic lives here.
 */
import {
  renderOsc, renderNoise, lowPass, highPass, bandPass,
  mixLayers, normalize, expDecay, padTo, clamp,
} from './samplerRenderUtils.js';

// === 轻击：短促清脆，快速衰减 — A/B按钮打击感 ===
// v2: 增加短促高频成分，更像拳击手套击打的"啪"声
export function renderHitLight(sr: number): Float32Array {
  const dur = 0.075;
  // 中频噪声 — 紧凑肉体冲击
...
export function renderHitHeavy(sr: number): Float32Array {
  const dur = 0.25;
  // 低频噪声带 — 深沉肉体冲击
...
export function renderBlock(sr: number, heavy: boolean): Float32Array {
...
export function renderSpecial(sr: number): Float32Array {
  const dur = 0.3;
  // 能量蓄积呼啸
...
// v2: 增加持续时间，更戏剧化的能量爆发
export function renderDM(sr: number): Float32Array {
  const dur = 0.85;
  // 低频扫频 — 能量蓄积
...
// v3: 添加初始爆裂+延迟余震双重爆炸层
export function renderKO(sr: number): Float32Array {
  const dur = 1.2;
  // 主锯齿波 — 大幅下降
...
export function renderCounter(sr: number): Float32Array {
  const dur = 0.3;
  // 尖锐方波瞬态 — 更高频率
...
export function renderGuardCrush(sr: number): Float32Array {
  const dur = 0.15;
  // 白噪声 — 金属碎裂的主体质感
...
export function renderChip(sr: number): Float32Array {
  const dur = 0.05;
...
export function renderWallBounce(sr: number): Float32Array {
  const dur = 0.1;
...
export function renderGroundBounce(sr: number): Float32Array {
  const dur = 0.1;
  // 中频弹跳(300Hz) — 地面反弹的主体音
...
export function renderCancel(sr: number): Float32Array {
  const dur = 0.13;
...
export function renderWire(sr: number): Float32Array {
  const dur = 0.16;
  const osc = renderOsc(sr, dur, 'sawtooth',
...
export function renderJuggle(sr: number): Float32Array {
  const dur = 0.05;
...
export function renderSuperFlash(sr: number, isSDM: boolean): Float32Array {
...
export function renderSuperFlashHSDM(sr: number): Float32Array {
  const dur = 0.65;
  // 主振荡器: 更低沉的扫频 + 更长的持续
  const osc = renderOsc(sr, dur, 'sawtooth',
...
export function renderThrow(sr: number): Float32Array {
  const dur = 0.16;
...
export function renderThrowEscape(sr: number): Float32Array {
  const dur = 0.12;
...
export function renderSelect(sr: number): Float32Array {
  const dur = 0.2;
...
export function renderCursorMove(sr: number): Float32Array {
  const dur = 0.06;
  const osc = renderOsc(sr, dur, 'sine', _t => 1200, t => expDecay(t, 0.06, 30));
  return normalize(osc);
}

export function renderCursorConfirm(sr: number): Float32Array {
  const dur = 0.15;
  const osc = renderOsc(sr, dur, 'sine',
    t => t < 0.04 ? 660 : 990,
    t => expDecay(t, 0.15, 18));
...
export function renderVictory(sr: number): Float32Array {
...
export function renderRoll(sr: number): Float32Array {
  const dur = 0.15;
  const osc = renderOsc(sr, dur, 'sine',
...
export function renderLanding(sr: number): Float32Array {
  const dur = 0.09;
  // 低频着地冲击 — 更明显的"砰"
...
export function renderProjectile(sr: number): Float32Array {
  const dur = 0.2;
  const osc = renderOsc(sr, dur, 'sawtooth',
...
export function renderMAXActivation(sr: number): Float32Array {
  const dur = 0.6;
  const osc = renderOsc(sr, dur, 'sawtooth',
...
export function renderRoundCall(sr: number): Float32Array {
...
export function renderTimeOver(sr: number): Float32Array {
...
export function renderPerfect(sr: number): Float32Array {
...
export function renderFight(sr: number): Float32Array {
...
export function renderQuickStand(sr: number): Float32Array {
  const dur = 0.12;
  const osc = renderOsc(sr, dur, 'triangle',
...
export function renderStep(sr: number): Float32Array {
  const dur = 0.04;
...
// 暴击：尖锐 + 共鸣 + 双层噪声
export function renderHitCrit(sr: number): Float32Array {
  const dur = 0.2;
  // 高频裂纹瞬态
...
// 灰尘/地面喷射声
export function renderDust(sr: number): Float32Array {
  const dur = 0.12;
  // 低通噪声 — 柔和的空气喷射
...
// 空中打击：轻盈 + 更多呼啸感
export function renderAirHit(sr: number): Float32Array {
  const dur = 0.07;
  // 高频噪声 — 空气切割感
...
// 强力壁弹：金属撞击 + 碎片 + 剧烈低频
export function renderWallBounceHeavy(sr: number): Float32Array {
  const dur = 0.3;
  // 强金属撞击
...
// 防御崩坏：碎裂 + 能量释放
export function renderGuardBreak(sr: number): Float32Array {
  const dur = 0.3;
  // 尖锐裂纹 — 初始破裂
...
// 气槽充能：能量上升 + 脉冲感
export function renderChargeUp(sr: number): Float32Array {
  const dur = 0.4;
  // 上升正弦波 — 能量蓄积
...
export function renderBlockSpecial(sr: number): Float32Array {
  const dur = 0.14;
...
export function renderBlockDM(sr: number): Float32Array {
  const dur = 0.25;
...
// 弱必杀命中：更轻更短的必杀技音效
export function renderSpecialLight(sr: number): Float32Array {
  const dur = 0.22;
...
export function renderSpecialHeavy(sr: number): Float32Array {
  const dur = 0.35;
  const whoosh = renderOsc(sr, dur, 'sawtooth',
    t => t < 0.1 ? 200 + 8000 * t : 1000 - 3500 * (t - 0.1),
    t => t < 0.1 ? 0.15 + t * 0.8 : expDecay(t - 0.1, 0.25, 7));
...
export function renderKOHit(sr: number): Float32Array {
  const dur = 0.5;
  // 初始冲击裂纹 — 极尖锐瞬态
...
export function renderLandingHeavy(sr: number): Float32Array {
  const dur = 0.2;
  // 深沉低频着地冲击
...
export function renderAccentFire(sr: number): Float32Array {
  const dur = 0.18;
  // 火焰噪声 — 中高频不规则噼啪
...
export function renderAccentPurple(sr: number): Float32Array {
  const dur = 0.2;
  // 能量嗡鸣 — 低中频
...
// 冰晶声音 — Kula等冰属性角色
export function renderAccentIce(sr: number): Float32Array {
  const dur = 0.2;
  // 高音冰晶碎裂 — 更高音调的结晶质感
...
export function renderAccentGeneric(sr: number): Float32Array {
  const dur = 0.12;
  // 中频能量脉冲
...
// 特征：延迟回声 + 空洞共鸣 + 眩晕感
export function renderDizzyHit(sr: number): Float32Array {
  const dur = 0.35;
  // 主冲击 — 中频噪声
...
export function renderPerfectKO(sr: number): Float32Array {
  const dur = 1.5;
  const total = Math.ceil(sr * dur);
...
// 用于回合开始时的倒计时结束提示音
export function renderRoundStart(sr: number): Float32Array {
  const dur = 0.5;
  const total = Math.ceil(sr * dur);
...
// Time Up：下降音阶三角波 + 铃声
// 用于时间耗尽时的提示音
export function renderTimeUp(sr: number): Float32Array {
  const dur = 0.8;
  const total = Math.ceil(sr * dur);
...
export function renderWhoosh(sr: number): Float32Array {
  const dur = 0.07;
  // 高频噪声带 — 空气切割主体
...
export function renderWhooshHeavy(sr: number): Float32Array {
  const dur = 0.14;
  // 低频体感 — 重挥拳的"呼"声底层
...
export function renderFootstep(sr: number): Float32Array {
  const dur = 0.06;
  // 脚跟冲击 — 短促低频"咚"
...
export function renderJump(sr: number): Float32Array {
  const dur = 0.1;
  // 上升正弦波 — 离地时的发力感
...
export function renderLandingNormal(sr: number): Float32Array {
  const dur = 0.12;
  // 脚尖着地 — 第一段较轻的低频
...
export function generateBattleBGM(sr: number): Float32Array {
  const bpm = 120;
