...
import { loadRealSprites, resolveKyoMugenAction } from '../shared/realSpriteLoader.js';
...
// Backward-compatible named exports — delegate to mutable renderer
export function hasKyoHighResFrame(state: FighterState, attack: AttackType | null, vx: number, facing: number): boolean {
  return currentRenderer().has(state, attack, vx, facing);
}
export function drawKyoHighResFrame(ctx: CanvasRenderingContext2D, state: FighterState, stateAge: number, x: number, y: number, facing: number, attack: AttackType | null, vx: number): boolean {
  return currentRenderer().draw(ctx, state, stateAge, x, y, facing, attack, vx);
}
export function drawKyoHighResAfterimage(ctx: CanvasRenderingContext2D, state: FighterState, stateAge: number, x: number, y: number, facing: number, attack: AttackType | null, vx: number, tint?: string, alpha?: number): boolean {
  return currentRenderer().drawAfterimage(ctx, state, stateAge, x, y, facing, attack, vx, tint, alpha);
}
export function drawKyoWinPose(ctx: CanvasRenderingContext2D, stateAge: number, x: number, y: number, facing: number): boolean {
  return currentRenderer().drawWinPose(ctx, stateAge, x, y, facing);
}

// ===== Real Sprite Integration =====
// After calling initKyoRealSprites(), the renderer will use real MUGEN
// pixel art PNGs instead of procedural pixel frames.

let activeRenderer: ReturnType<typeof createHighResRenderer> = renderer;
let realSpriteInitialized = false;

function currentRenderer() { return activeRenderer; }

/**
 * Load real MUGEN sprites for Kyo and inject them into the renderer.
 * After this, drawKyoHighResFrame() will draw real pixel art.
 * Falls back to procedural frames for any missing actions.
 */
export async function initKyoRealSprites(): Promise<void> {
  if (realSpriteInitialized) return;

  try {
    const imageFrames = await loadRealSprites(
      '/sprites/cvskyo/manifest.json',
      '/sprites/cvskyo',
    );

    // Create a new renderer that has both procedural (fallback) and real sprites
    const enhancedRenderer = createHighResRenderer({
      targetDisplayHeight: 96,
      defaultTint: '#ff6600',
      setup(reg, regV) {
        // Keep all procedural frames as fallback
        regV('IDLE', KYO_IDLE_FRAMES, [8, 9, 12, 9, 10, 8]);
        regV('WALK_FORWARD', KYO_WALK_FORWARD_FRAMES, [6, 5, 6, 6, 5, 6]);
        regV('WALK_BACKWARD', KYO_WALK_BACKWARD_FRAMES, [7, 6, 7, 7, 6, 7]);
        regV('STAND_A', KYO_STAND_A_FRAMES, [6, 3, 2, 5]);
      ...
      resolveKey: resolveKyoMugenAction,
      imageSetup(regImg) {
    ...
    console.log('[Kyo] Real sprites loaded:', imageFrames.size, 'actions');
  } catch (e) {
    console.warn('[Kyo] Real sprite loading failed, using procedural fallback:', e);
  }
}
