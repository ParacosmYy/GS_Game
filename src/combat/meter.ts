...
function meterGainForAttack(base: number, attackType: AttackType): number {
  ...
  if (name.startsWith('DM_') || name.startsWith('SDM_') || name.startsWith('HSDM_')) return Math.round(base * 2.0);
  if (name.startsWith('KYO_') || name.startsWith('IORI_') || name.startsWith('TERRY_')
    || name.startsWith('KIM_') || name.startsWith('RYO_') || name.startsWith('LEONA_') || name.startsWith('KDASH_') || name.startsWith('KULA_')
    || name === 'SPECIAL_UPPER' || name === 'SPECIAL_PROJECTILE') return Math.round(base * 1.5);
