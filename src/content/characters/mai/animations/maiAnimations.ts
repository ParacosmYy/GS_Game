/**
 * Mai Content Package — Animation Metadata
 *
 * 动作帧元数据: 动作列表、帧数、循环属性、过渡类型。
 * 实际帧序列数据来自 core/animationManifestData.ts, 此文件只放元数据描述层。
 *
 * 归属: content/characters/mai/animations/ — 只放"动作帧是什么"
 */
...
export const MAI_ANIMATION_META: Record<string, AnimationMeta> = {
  // === 基础动作 ===
  idle:           { name: 'idle', type: 'loop', totalFrames: 6, ticksPerFrame: 8, loop: true, transition: 'blend', description: '站立待机,不知火流忍术基本架势' },
  walk_forward:   { name: 'walk_forward', type: 'loop', totalFrames: 6, ticksPerFrame: 6, loop: true, transition: 'ease_in', description: '前进移动,忍者步伐' },
  ...
  // === 命令通常技 ===
  mai_hissatsu_shinobibachi:  { name: 'mai_hissatsu_shinobibachi', type: 'attack', totalFrames: 40, ticksPerFrame: 2, loop: false, transition: 'snap', description: '必殺忍蜂,上段忍蜂击' },
  mai_yusura_uma:             { name: 'mai_yusura_uma', type: 'attack', totalFrames: 34, ticksPerFrame: 2, loop: false, transition: 'snap', description: '夕櫻舞,下段踢' },

  // === 必杀技 ===
  mai_kachousen:    { name: 'mai_kachousen', type: 'attack', totalFrames: 38, ticksPerFrame: 2, loop: false, transition: 'snap', description: '花蝶扇A版,扇形飞行道具' },
  mai_kachousen_c:  { name: 'mai_kachousen_c', type: 'attack', totalFrames: 46, ticksPerFrame: 2, loop: false, transition: 'snap', description: '花蝶扇C版,强扇形飞行道具' },
  mai_ryuuenbu:     { name: 'mai_ryuuenbu', type: 'attack', totalFrames: 42, ticksPerFrame: 2, loop: false, transition: 'snap', description: '龍炎舞,火焰扇击' },
  mai_ryuenjin:     { name: 'mai_ryuenjin', type: 'attack', totalFrames: 44, ticksPerFrame: 2, loop: false, transition: 'snap', description: '飛翔龍炎陣,火焰升龙' },

  // === DM ===
  dm_haka_otoshi:  { name: 'dm_haka_otoshi', type: 'attack', totalFrames: 62, ticksPerFrame: 2, loop: false, transition: 'snap', description: '蜂巢落としDM,超必杀扇舞' },

  // === SDM ===
  sdm_haka_otoshi: { name: 'sdm_haka_otoshi', type: 'attack', totalFrames: 68, ticksPerFrame: 2, loop: false, transition: 'snap', description: '蜂巢落としSDM,强化超必杀扇舞' },
};

/** 获取Mai所有动作名称 */
export function getMaiAnimationNames(): string[] {
  return Object.keys(MAI_ANIMATION_META);
}

/** 获取指定动作元数据 */
export function getMaiAnimMeta(name: string): AnimationMeta | undefined {
  return MAI_ANIMATION_META[name];
}

/** 获取攻击动作列表 */
export function getMaiAttackAnimations(): AnimationMeta[] {
  return Object.values(MAI_ANIMATION_META).filter(m => m.type === 'attack');
}

/** 获取循环动作列表 */
export function getMaiLoopAnimations(): AnimationMeta[] {
  return Object.values(MAI_ANIMATION_META).filter(m => m.loop);
}
