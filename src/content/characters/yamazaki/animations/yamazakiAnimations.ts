/**
 * Yamazaki Content Package — Animation Metadata
 *
 * 动作帧元数据: 动作列表、帧数、循环属性、过渡类型。
 * 实际帧序列数据来自 core/animationManifestData.ts, 此文件只放元数据描述层。
 *
 * 归属: content/characters/yamazaki/animations/ — 只放"动作帧是什么"
 *
 * Ryuji Yamazaki: one-handed snake-arm fighter, Orochi bloodline.
 * Idle stance: right hand in pocket, left arm loose, menacing sway.
 */
...
export const YAMAZAKI_ANIMATION_META: Record<string, AnimationMeta> = {
  // === 基础动作 ===
  idle:           { name: 'idle', type: 'loop', totalFrames: 6, ticksPerFrame: 8, loop: true, transition: 'blend', description: '站立待机,右手插口袋,左手自然下垂,身体微微晃动' },
  walk_forward:   { name: 'walk_forward', type: 'loop', totalFrames: 6, ticksPerFrame: 6, loop: true, transition: 'ease_in', description: '前进移动,沉重步伐,手不离开口袋' },
  walk_backward:  { name: 'walk_backward', type: 'loop', totalFrames: 6, ticksPerFrame: 7, loop: true, transition: 'ease_in', description: '后退移动,保持压迫感' },
  run:            { name: 'run', type: 'loop', totalFrames: 2, ticksPerFrame: 4, loop: true, transition: 'ease_in', description: '跑步前冲,大步流星' },
  ...
  // === 通常技 ===
  stand_a:  { name: 'stand_a', type: 'attack', totalFrames: 14, ticksPerFrame: 2, loop: false, transition: 'snap', description: '远A轻拳,单手直刺' },
  stand_b:  { name: 'stand_b', type: 'attack', totalFrames: 24, ticksPerFrame: 2, loop: false, transition: 'snap', description: '远B轻踢' },
  stand_c:  { name: 'stand_c', type: 'attack', totalFrames: 30, ticksPerFrame: 2, loop: false, transition: 'snap', description: '远C重拳,蛇臂延伸' },
  ...
  // === 胜利/系统 ===
  win:      { name: 'win', type: 'once', totalFrames: 8, ticksPerFrame: 10, loop: false, transition: 'ease_out', description: '胜利姿势,狂笑' },
  taunt:    { name: 'taunt', type: 'once', totalFrames: 6, ticksPerFrame: 10, loop: false, transition: 'ease_out', description: '挑衅,吹口哨' },
  max_mode: { name: 'max_mode', type: 'once', totalFrames: 4, ticksPerFrame: 6, loop: false, transition: 'snap', description: '爆气MAX Mode' },

  // === 命令通常技 ===
  yamazaki_sashi: { name: 'yamazaki_sashi', type: 'attack', totalFrames: 25, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Sashi 刺し,上段直刺' },
  yamazaki_bokkai: { name: 'yamazaki_bokkai', type: 'attack', totalFrames: 28, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Bokkai 暴会,下段突进' },

  // === 必杀技 ===
  yamazaki_snake_arm:    { name: 'yamazaki_snake_arm', type: 'attack', totalFrames: 36, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Snake Arm A版,蛇臂延伸刺击' },
  yamazaki_snake_arm_c:  { name: 'yamazaki_snake_arm_c', type: 'attack', totalFrames: 46, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Snake Arm C版,更强蛇臂延伸' },
  yamazaki_sandstorm:    { name: 'yamazaki_sandstorm', type: 'attack', totalFrames: 46, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Sandstorm 爆弾拳,中段爆发打击' },
  yamazaki_bai_ga_se:    { name: 'yamazaki_bai_ga_se', type: 'attack', totalFrames: 38, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Bai Gaeshi 倍返し,下段扫腿反击' },
  yamazaki_snake_arm_qcf: { name: 'yamazaki_snake_arm_qcf', type: 'attack', totalFrames: 32, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Snake Arm follow-up 追加刺击' },
  yamazaki_drill:         { name: 'yamazaki_drill', type: 'attack', totalFrames: 50, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Drill ドリル,多段连续钻击' },

  // === DM ===
  dm_guillotine:  { name: 'dm_guillotine', type: 'attack', totalFrames: 64, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Guillotine DM ギロチン,断头台级重击' },

  // === SDM ===
  sdm_guillotine: { name: 'sdm_guillotine', type: 'attack', totalFrames: 66, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Guillotine SDM 强化版ギロチン' },

  // === HSDM ===
  hsdm_drill:     { name: 'hsdm_drill', type: 'attack', totalFrames: 65, ticksPerFrame: 2, loop: false, transition: 'snap', description: 'Drill HSDM 终极ドリル' },
};

/** 获取Yamazaki所有动作名称 */
export function getYamazakiAnimationNames(): string[] {
  return Object.keys(YAMAZAKI_ANIMATION_META);
}

/** 获取指定动作元数据 */
export function getYamazakiAnimMeta(name: string): AnimationMeta | undefined {
  return YAMAZAKI_ANIMATION_META[name];
}

/** 获取攻击动作列表 */
export function getYamazakiAttackAnimations(): AnimationMeta[] {
  return Object.values(YAMAZAKI_ANIMATION_META).filter(m => m.type === 'attack');
}

/** 获取循环动作列表 */
export function getYamazakiLoopAnimations(): AnimationMeta[] {
  return Object.values(YAMAZAKI_ANIMATION_META).filter(m => m.loop);
}
