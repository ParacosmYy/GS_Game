...
function pickWinQuote(w: number | null): string { return _pickWinQuote(w, p1, p2); }
  ...
  if (gs.phase === GamePhase.CONTINUE) {
    ...
    if (inputManager.isKeyDown('KeyJ') || inputManager.isKeyDown('Enter')) {
      if (gs.continueCursorYes) {
        ...
        gs.gameOverTimer = 0;
        announcer.gameOver();
        screenFlash.trigger('#220000', 0.3, 15);
...
function render(): void {
  ...
  // KOF2002: Character-specific intro animations during INTRO phase
  if (gs.phase === GamePhase.INTRO) {
