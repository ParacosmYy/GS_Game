 ...
 *
 * 必杀技:
 *   QCF+A/C → Outrage (突进撕抓, weak/strong)
 *   QCB+P   → Black End (抓取砸地)
 *   HCF+K   → Mayhem (下段突进)
 * DM: QCFx2+P → Negative Gain (多段撕抓连击)
 */
...
import { vicePortrait } from '../rendering/portraits/vicePortrait.js';

export const ViceDef: CharacterDefinition = {
  id: 'vice',
  name: 'VICE',
  nameCn: '薇丝',
  color: '#3366aa',
  ...
  specialGlow: '#77bbff',
  portrait: '♀',
  pixelPortrait: vicePortrait,
  winQuotes: ['大蛇之力……感觉到了吗？', '你的挣扎毫无意义。', '这就是背叛大蛇的下场。'],
  ...
  poses: {
    ...
    // — Attack poses (Violent/grappling style: open palm strikes, claw grabs) —
  ...
  routeSpecial(input, cmdBuf, tick, _hasChargeRelease = false) {
    // DM: QCFx2+P → Negative Gain
    const dmMotion = cmdBuf.checkDMMotion(tick, input.punchPressed, input.kickPressed);
    if (dmMotion === 'QCFx2_P') return AttackType.DM_NEGATIVE_GAIN;

    // HCF+K → Mayhem (low rush)
    if (input.kickPressed && cmdBuf.hasQCB(tick)) {
      return AttackType.VICE_MAYHEM;
    }

    // QCB+P → Black End (overhead grab slam)
    if (input.punchPressed && cmdBuf.hasQCB(tick)) {
      return AttackType.VICE_BLACK_END;
    }

    // QCF+P → Outrage (weak/strong rush)
    const special = cmdBuf.checkSpecial(tick, input.punchPressed || input.kickPressed);
    if (special === AttackType.SPECIAL_PROJECTILE) {
      return input.buttonCPressed ? AttackType.VICE_OUTRAGE_C : AttackType.VICE_OUTRAGE;
    }
  ...
  routeNormal(input, state, _isCloseRange) {
    ...
    // →+A Monstrosity (upper)
    if (!isAir && input.buttonAPressed && input.forward && !input.down) return AttackType.VICE_MONSTROSITY;
    // →+B Overkill (low)
    if (!isAir && input.buttonBPressed && input.forward && !input.down) return AttackType.VICE_OVERKILL;
    return null;
  },
