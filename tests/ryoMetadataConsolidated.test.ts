/**
 * Ryo Data & Stats Consolidated Tests
 *
 * Merged from: ryoData, ryoContentData, ryoStats
 *
 * Covers: RYO_CHARACTER_DATA metadata, RYO_STATS stats completeness.
 */
...
describe('RYO_STATS', () => {
  ...
  it('has numeric fields', () => {
    const fields = ['walkSpeed', 'runSpeed', 'jumpVelocity', 'maxHealth', 'hopVelocity'] as const;
    for (const f of fields) {
      expect(typeof (RYO_STATS as any)[f], `RYO_STATS.${f}`).toBe('number');
    }
  });
});
